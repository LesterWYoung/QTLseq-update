...
if [ ! -d "${Key1_Bulked_sample_name}_${Key1_Bulked_sample_Type_A}" ]; then
	mv mybulk_A "${Key1_Bulked_sample_name}_${Key1_Bulked_sample_Type_A}"
fi
if [ ! -d "${Key1_Bulked_sample_name}_${Key1_Bulked_sample_Type_B}" ]; then
	mv mybulk_B "${Key1_Bulked_sample_name}_${Key1_Bulked_sample_Type_B}"
fi

MKDIR="mkdir -p"			
						
if [ ! -d "${bulk_name_ida}" ] ; then			
	CMD="${MKDIR} ${bulk_name_ida}"		
	echo ${CMD}		
#	eval ${CMD}		
fi			
			
if [ ! -d "${bulk_name_idb}" ] ; then			
...
if [[ ${Key0_Make_secondary_reference} = "yes/no" ]] ; then
	...
	if [[ ${Key0_Make_secondary_reference} = "yes" ]]; then
		if [[ ${Key1_My_cultivar_sample_name} = "INSERT-secondary-ref-name-OR-leave-empty" ]]; then
			echo "Change Key1_My_cultivar_sample_name to secondary reference name in config.txt"
			exit 2
		else
			if [[ -z "${Key1_My_cultivar_sample_name}" ]]; then
				echo "Insert value for Key1_My_cultivar_sample_name in config.txt"
				exit 3 	# user need to create diectory to put secondary reference readfiles
			else		
				if [ -d parental_readfiles ]; then	
					mv parental_readfiles "${Key1_My_cultivar_sample_name}"
					# change parental_readfiles dir to name of secondary ref cultivar					
				else
					if [ ! -d ${Key1_My_cultivar_sample_name} ]; then
						echo "Unable to find ./parental_readfiles/ or ./${Key1_My_cultivar_sample_name}/"
						echo "Readfiles for secondary reference should be in one of these directories."
						exit 4
					fi
				fi	# assumes that parental readfiles are now in dir named Key1_My_cultivar_sample_name
	
				if [ ! -d ${trimmed_parental_readfile_dir} ]; then
					echo ${trimmed_parental_readfile_dir}
					eval mkdir ${trimmed_parental_readfile_dir}
				fi
				
				if [ ! -d ${secondary_ref_dir} ]; then
					echo ${secondary_ref_dir}
					eval mkdir ${secondary_ref_dir}
				fi
			fi
		fi
	fi
